  ...
  return requireAdmin(req, async () => {
    try {
      ...
      const analytics = {
        ...
        users: {
          uniquePayingUsers: new Set(
            transactions.filter((t) => t.status === 'completed').map((t) => t.userId)
          ).size,
          subscriptionUsers: transactions.filter(
            (t) => t.status === 'completed' && t.type === 'subscription'
          ).length,
          oneTimePurchaseUsers: transactions.filter(
            (t) => t.status === 'completed' && t.type === 'purchase'
          ).length,
        },
        currency: {
      ...
      completedTransactions.forEach((t) => {
        ...
        const amount = t.currency === 'IRT' ? t.amount / 70000 : t.amount;
        analytics.revenue.revenueByMonth[monthKey] =
          (analytics.revenue.revenueByMonth[monthKey] || 0) + amount;
      });
      ...
      completedTransactions.forEach((t) => {
        if (t.packageId) {
          const amount = t.currency === 'IRT' ? t.amount / 70000 : t.amount;
          analytics.revenue.revenueByPackage[t.packageId] =
            (analytics.revenue.revenueByPackage[t.packageId] || 0) + amount;
        } else {
          const amount = t.currency === 'IRT' ? t.amount / 70000 : t.amount;
          analytics.revenue.revenueByPackage['custom'] =
            (analytics.revenue.revenueByPackage['custom'] || 0) + amount;
        }
      });
      ...
      completedTransactions.forEach((t) => {
        if (t.packageId) {
          analytics.tokens.tokensByPackage[t.packageId] =
            (analytics.tokens.tokensByPackage[t.packageId] || 0) + (t.tokens || 0);
        } else {
          analytics.tokens.tokensByPackage['custom'] =
            (analytics.tokens.tokensByPackage['custom'] || 0) + (t.tokens || 0);
        }
      });
