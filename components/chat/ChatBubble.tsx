...
interface ChatBubbleProps {
  ...
  children?: ReactNode;
  animate?: boolean;
  enableTyping?: boolean; // Enable typing animation for assistant messages
  typingSpeed?: number; // Characters per second (default: 40)
}
...
export function ChatBubble({
    ...
    return (
      <div className={`flex justify-end ${animate ? 'animate-slideUp' : ''}`}>
        <div className="max-w-[90%] bg-gradient-brand rounded-xl rounded-tr-sm px-5 py-3 shadow-lg">
          <p className="text-sm leading-relaxed text-white font-medium break-words overflow-wrap-anywhere">
            {content}
          </p>
        </div>
      </div>
  ...
  // Assistant messages - role-based icons
  if (type === 'assistant') {
    // Extract role from message if formatted as "**Role**: message" or detect from keywords
    const getRoleAndIcon = () => {
      // Check for role header format: "**Product Manager**: ..."
      const roleHeaderMatch = content.match(/\*\*([^:]+)\*\*:/);
      const role = roleHeaderMatch ? roleHeaderMatch[1].toLowerCase() : '';

      const lowerContent = content.toLowerCase();

      // PRIORITY 1: Role-based icons (monochrome, professional)
      ...
      if (role.includes('frontend engineer') || role.includes('frontend')) {
        return {
          icon: (
            <svg
              className="w-4 h-4 text-gray-600 dark:text-gray-300"
      ...
      if (role.includes('backend engineer') || role.includes('backend')) {
        return {
          icon: (
            <svg
              className="w-4 h-4 text-gray-600 dark:text-gray-300"
      ...
      if (role.includes('ux designer') || role.includes('designer')) {
        return {
          icon: (
            <svg
              className="w-4 h-4 text-gray-600 dark:text-gray-300"
      ...
      if (role.includes('managing director') || role.includes('founder')) {
        return {
          icon: (
            <svg
              className="w-4 h-4 text-gray-600 dark:text-gray-300"
      ...
      if (role.includes('qa engineer') || role.includes('quality')) {
        return {
          icon: (
            <svg
              className="w-4 h-4 text-gray-600 dark:text-gray-300"
      ...
      // PRIORITY 2: Check for edit success messages - always show as success (green)
      if (
        lowerContent.includes("here's what i changed") ||
        lowerContent.includes("here's what changed") ||
        lowerContent.includes('edited files') ||
        lowerContent.includes('added features')
      ) {
        return {
          icon: (
            <svg
      ...
      if (
        ...
        lowerContent.includes('layout') ||
        lowerContent.includes('styling')
      ) {
        return {
          icon: (
            <svg
      ...
      if (
        ...
        lowerContent.includes('understanding') ||
    ...
    const { icon, bgClass, isSuccess, isError, isInfo, isWarning, roleIcon } = getRoleAndIcon();

    return (
      <div className={`flex justify-start ${animate ? 'animate-slideUp' : ''}`}>
        <div
          className={`max-w-[90%] rounded-xl rounded-tl-sm px-5 py-3 shadow-md ${
            roleIcon
              ? 'bg-background-raised border border-light' // Neutral for role messages
              : isError
        ...
        >
          <div className="flex items-start gap-3">
            <div
              className={`w-7 h-7 rounded-lg ${bgClass} flex items-center justify-center flex-shrink-0 shadow-md`}
            >
              {icon}
            </div>
            <div
              className={`flex-1 min-w-0 text-sm leading-relaxed prose prose-sm max-w-none break-words overflow-wrap-anywhere ${
                isError
            ...
            >
              {enableTyping ? (
                <TypingText text={content} speed={typingSpeed} showCursor={true} />
              ) : (
    ...
    return (
      <div className="flex justify-start animate-fadeIn">
        <div className="flex items-center gap-2.5 px-2 py-1">
          ...
          <span className="text-sm text-text-secondary font-medium">{content}</span>

          {/* Slower 3-dot animation - bouncing up and down */}
          <div className="flex items-center gap-0.5 ml-0.5">
            <div
              className="w-1 h-1 bg-text-tertiary rounded-full animate-bounce"
    ...
    return (
      <div className={`flex justify-center ${animate ? 'animate-slideUp' : ''}`}>
        <div className="bg-success/5 border border-success rounded-xl px-6 py-4 shadow-md max-w-[95%]">
          ...
          {children && (
            <div className="mt-3 flex gap-3 justify-end pt-3 border-t border-light">{children}</div>
          )}
        </div>
      </div>
    );
