              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-text-tertiary mt-0.5">
                    {actualCompleteLog &&
                      `Completed in ${(actualCompleteLog.duration! / 1000).toFixed(1)}s`}
              ...
              {actualCompleteLog && (
                <div className="text-sm text-text-secondary mb-2 pl-11">
                  {nodeName === 'devops' ? (
                    ...
                    <Markdown
                      content={
                        // Priority: dynamic summary > fallback message > generic
                        actualCompleteLog.taskDetails?.summary ||
                        ROLE_COMPLETE_MESSAGES[nodeName] ||
              ...
              {nodeName === 'frontend' && fileProgress && fileProgress.totalFiles > 0 && (
                <div className="mt-3 pl-11 space-y-1">
                  <div className="text-xs font-medium text-text-tertiary mb-2">
                    {errorLog
                      ? `Failed creating files (${fileProgress.createdFiles.length}/${fileProgress.totalFiles})`
                      : `Creating ${fileProgress.totalFiles} files...`}
                  </div>
                    ...
                    {Array.from({ length: fileProgress.totalFiles }).map((_, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-2 text-xs">
                          ...
                          <span
                            className={`font-mono ${isCreated ? 'text-text-secondary' : isCurrent ? 'bg-gradient-blue bg-clip-text text-transparent' : 'text-text-tertiary opacity-50'}`}
                          >
                            {fileName || `File ${idx + 1}`}
                          </span>
                        </div>
                    ...
                    return (
                      <div key={agentRole} className="flex items-center gap-2 text-xs">
                        ...
                        {isAgentActive && agentStartLog.message && (
                          <span className="text-text-tertiary ml-1">- {agentStartLog.message}</span>
                        )}
                      </div>
                    );
                  })}
        ...
        <div className="bg-background-raised border border-light rounded-xl p-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-text-tertiary mt-0.5">
                Initializing workflow with AI team
              </div>
            </div>
            <div className="flex gap-1">
              <div
