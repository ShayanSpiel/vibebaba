      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-background-raised to-background-subtle border-b border-light shadow-sm">
        ...
        <div className="flex-1 relative flex items-center gap-2" ref={dropdownRef}>
          <div
            ...
            onClick={() =>
              availableRoutes.length > 0 && deploymentUrl && setShowUrlDropdown(!showUrlDropdown)
            }
          >
            <svg
            ...
            <input
              ...
              value={
                deploymentUrl
                  ? (deploymentUrl + currentPath).replace(/([^:]\/)\/+/g, '$1')
                  : isDeploying
                    ? 'Deploying...'
          ...
          {deploymentUrl && (
            <button
              onClick={() =>
                window.open(previewUrl || deploymentUrl, '_blank', 'noopener,noreferrer')
              }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <svg
                          className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-brand-primary' : 'text-text-tertiary group-hover:text-brand-primary'}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <div
                            className={`text-sm font-medium ${isActive ? 'text-brand-primary' : 'text-text-primary'} mb-0.5`}
                          >
                            {route.path}
                          </div>
                          <div className="text-xs text-text-tertiary">{route.label}</div>
                        </div>
                        {isActive && (
          ...
          <div className="flex items-center justify-center absolute inset-0 bg-background-base z-10">
            <div className="text-center">
              ...
              <p className="text-sm text-text-primary font-bold mb-2">
                {isDeploying ? loadingMessage : 'Loading your app...'}
              </p>
              <p className="text-xs text-text-tertiary">
                {isDeploying ? 'Setting up real server & database' : 'Preview coming up...'}
              </p>
            </div>
          </div>
        ) : null}
