...
interface PreviewTabsProps {
  ...
  onDeploymentStatusChange?: (status: 'idle' | 'deploying' | 'deployed' | 'failed', error?: string) => void;
}

export default function PreviewTabs({ project, onUpdateProject, activeView, onDeploymentStatusChange }: PreviewTabsProps) {
  ...
  useEffect(() => {
    if (deployment.status === 'deployed' && deployment.filesDeployed && deployment.filesDeployed > 0) {
  ...
  useEffect(() => {
    ...
    const prepareDeploymentFiles = () => {
      ...
      console.log(`[PreviewTabs] 📦 Prepared ${allFiles.length} files for deployment (${project.files.length} user + ${scaffoldFiles.length} scaffold)`);
    ...
    if (deployment.status === 'idle' && !hasDeployedOnce.current) {
      ...
      console.log('🚀 [PreviewTabs] Initial deployment - deploying', deploymentFiles.length, 'files');
  ...
  return (
    <div className="h-full flex flex-col bg-background-base">
              ...
              {!project.plan ? (
                <div className="h-full flex items-center justify-center">
                  <div className="text-center p-8 max-w-4xl mx-auto w-full">
                    ...
                    <p className="text-sm text-text-tertiary">AI is analyzing your idea and creating a comprehensive plan...</p>
                  </div>
                </div>
              ) : (
                <PlanPreview
              ...
              {project.loadingMessage ? (
                <div className="h-full flex items-center justify-center bg-background-base">
                  <div className="text-center max-w-md mx-auto px-4">
                    ...
                    <p className="text-sm text-text-tertiary">Grab a coffee ☕ Your startup is being born!</p>
                  </div>
                </div>
              ) : (
                <BrowserPreview
                ...
                {selectedFile ? (
                  ...
                  <div className="h-full flex items-center justify-center bg-background-raised border-l border-light">
                    <div className="text-center p-8">
                      ...
                      <p className="text-lg font-semibold text-text-primary">Select a file from the explorer</p>
                      <p className="text-sm text-text-secondary mt-2">Click on any file to start editing</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      <div className={`h-full ${activeView === "database" ? "" : "hidden"}`}>
        <div className="h-full">
          {(project.stage === "building" || project.stage === "completed") && project.backendConfig ? (
            <DatabaseViewerPro
            ...
            <div className="h-full flex items-center justify-center bg-background-base">
              <div className="text-center p-8">
                ...
                <p className="text-text-secondary text-lg font-medium">Database not available yet</p>
                <p className="text-sm text-text-tertiary mt-2">Confirm the plan to build your app with database</p>
              </div>
            </div>
          )}
        </div>
