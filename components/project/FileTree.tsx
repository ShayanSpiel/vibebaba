...
import { ChevronDown, ChevronUp, Copy, Edit2, FileImage, FileText, Trash2 } from 'lucide-react';
...
const FileTree = memo(
  function FileTree({
    ...
    const renderNode = useCallback(
      (node: FileNode, depth: number = 0): React.ReactElement | null => {
          ...
          return (
            <div key={node.path}>
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${isExpanded ? 'text-text-primary' : 'text-text-secondary'}`}
                >
                  {node.name}
                </span>
                {node.children && (
        ...
        return (
          ...
          >
            <div className={fileColor}>{fileIcon}</div>
            <span
              className={`text-sm truncate ${isSelected ? 'text-amber-500 font-medium' : 'text-text-primary'}`}
            >
              {node.name}
            </span>
            {isSelected && (
      ...
      <div className="h-full flex flex-col bg-background-raised border-r border-light">
        ...
        <div className="p-4 border-b border-light bg-gradient-to-br from-background-raised to-background-subtle">
          ...
          <div className="relative">
            <svg
              className="w-4 h-4 text-text-tertiary absolute left-3 top-1/2 -translate-y-1/2"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
          {fileTree.length > 0 ? (
            <div className="space-y-0.5">{fileTree.map((node) => renderNode(node, 0))}</div>
          ) : (
            <div className="flex flex-col items-center justify-center h-32 px-4 text-center">
              <svg
                className="w-12 h-12 text-text-tertiary mb-2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        {projectId && (
          <div className="border-t border-light bg-background-base">
            ...
            >
              ...
              <span className="text-xs text-text-tertiary mr-2">{uploadedFiles.length}</span>
              {showUploadedFiles ? (
                ...
                {uploadedFiles.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-6 px-4 text-center">
                    ...
                    <p className="text-text-tertiary text-xs mt-1">
                      Use the attachment button in chat
                    </p>
                  </div>
                )}
              </div>
