...
const ProjectItem = memo(
  function ProjectItem({
    ...
    const handleDelete = useCallback(
      (e: React.MouseEvent) => onDelete(project.id, e),
      [onDelete, project.id]
    );
...
export function ProjectsSidebar() {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      combinedProjects.sort(
        (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );

      if (combinedProjects.length > 0) {
        console.log(
          `[ProjectsSidebar] Loaded ${combinedProjects.length} projects (${userProjects.length} from PB, ${localProjects.length} from localStorage)`
        );
      }
  ...
  return (
    <>
      ...
      <div
        className={`fixed ${isRTL ? 'right-0 border-l' : 'left-0 border-r'} top-0 h-full w-80 bg-background-base border-light shadow-xl z-40 flex flex-col`}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto">
          {projects.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-text-tertiary">{t('startCreating')}</p>
            </div>
          ) : (
            <div className="p-3">
