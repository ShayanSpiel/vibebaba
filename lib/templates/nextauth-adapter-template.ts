// Template for PocketBase adapter (as string)
...
export function PocketBaseAdapter(pbUrl: string): Adapter {
  ...
  return {
    ...
    async getSessionAndUser(sessionToken: string): Promise<{
      ...
      try {
        const session = await pb.collection('sessions').getFirstListItem(
    ...
    async useVerificationToken({
      ...
      try {
        const record = await pb.collection('verification_tokens').getFirstListItem(
