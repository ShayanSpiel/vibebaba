...
export const MCP_SERVERS: Record<string, MCPServerConfig> = {
  ...
  filesystem: {
    ...
    env: {
      ...
      ALLOWED_DIRECTORIES: process.env.FILESYSTEM_ALLOWED_DIRS || "./templates,./generated,./components"
    }
  },
...
export class MCPClient {
  ...
  async connect() {
    ...
    try {
      ...
      console.warn(`[MCP] ⚠️ Failed to connect to ${this.serverName} - continuing without it`, error instanceof Error ? error.message : String(error));
  ...
  async callTool(toolName: string, args: Record<string, unknown>) {
    ...
    try {
      ...
      console.warn(`[MCP] ⚠️ Error calling tool ${toolName} on ${this.serverName} - returning null`, error instanceof Error ? error.message : String(error));
  ...
  async readResource(uri: string) {
    ...
    try {
      ...
      console.warn(`[MCP] ⚠️ Error reading resource ${uri} from ${this.serverName} - returning null`, error instanceof Error ? error.message : String(error));
      // Don't throw - return null so caller can handle gracefully
      return null;
    }
  }
...
export function formatToolsForPrompt(toolsByServer: Array<{ server: string; tools: any[] }>): string {
