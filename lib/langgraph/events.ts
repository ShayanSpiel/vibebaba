...
export function emitWorkflowComplete(state: AppGenState, totalDuration: number) {
  ...
  const validationFailed = state.validationResult?.status === 'fail' && (state.validationResult?.errors || []).length > 0;
  ...
  workflowEvents.emit('workflow:complete', {
    ...
    validationStatus: state.validationResult?.status || 'unknown', // NEW: Include validation status
    timestamp: new Date().toISOString()
  });
  // Console log removed - event streaming handles this
}
