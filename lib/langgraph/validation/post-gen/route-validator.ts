...
/**
 * Validate API routes completeness
...
export function validateApiRoutes(files: FileToValidate[], backendConfig?: any): ValidationError[] {
  ...
  if (!backendConfig?.apiEndpoints) {
    console.log('[RouteValidator] No API endpoints to validate');
    return errors;
  }
  ...
  for (const endpoint of apiEndpoints) {
    ...
    if (!routeExists) {
      errors.push({
        file: 'backend-config',
        line: 0,
        message: `API endpoint ${method} ${path} has no route file. Expected: ${expectedFilePath}`,
      ...
      console.log(`[RouteValidator] ✓ Route exists: ${expectedFilePath}`);
    }

    // Check API client function signature
    if (apiClientFile) {
      const hasFunction = apiClientFile.content.includes(`export async function ${handler}(`);
      if (!hasFunction) {
        errors.push({
          file: 'src/lib/api.ts',
          line: 0,
          message: `No API client function found for endpoint ${method} ${path} (expected: ${handler}())`,
  ...
  if (errors.length === 0) {
    console.log('[RouteValidator] ✅ All API routes are complete');
  } else {
    console.log(`[RouteValidator] ❌ Found ${errors.length} API route issues`);
  }
