...
export async function qaNode(state: AppGenState): Promise<Partial<AppGenState>> {
  ...
  try {
    ...
    emitNodeStart('qa', state, {
      ...
      interpretation:
        'Running automated quality checks on your generated code to ensure everything works correctly.',
    ...
    if (validationResult.report.errors.length > 0) {
      ...
      validationResult.report.errors.forEach((err, idx) => {
        console.log(
          `  ${idx + 1}. [${err.type}] ${err.message} ${err.file ? `in ${err.file}` : ''}`
        );
      });
    }
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    if (validationResult.report.errors.length > 0) {
      ...
      emitProgress(
        'qa',
        state.projectId,
        `Found ${errorCount} issue${errorCount > 1 ? 's' : ''}, calling debugging team! 🔧`
      );

      // Get expected pages for multi-page validation (from file paths)
      const expectedPages =
        state.files?.filter((f) => f.path.endsWith('page.tsx')).map((f) => f.path) || [];
      ...
      console.log(
        `[QA] ${debugResult.success ? '✅ SUCCESS' : '❌ FAILED'} after ${debugResult.attempts} attempt(s)`
      );
      ...
      console.log(
        `[QA] 📊 Fixed: ${validationResult.report.errors.length - debugResult.validationResult.report.errors.length}`
      );
      ...
      await messageManager.sendEvent(
        state.projectId,
        {
          ...
          criticalIssues: debugResult.success
            ? undefined
            : validationResult.report.errors.slice(0, 3).map((e) => e.message),
        },
        'qa'
      );
      console.log('[QA] 💬 Sent validation summary via MessageManager (now displays in UI!)');
      ...
      emitNodeComplete('qa', state, duration, {
        ...
        output: {
          ...
          fixed:
            debugResult.validationResult.report.errors.length <
            validationResult.report.errors.length,
        },
      ...
      return {
        ...
        errors: debugResult.success
          ...
          : [
              ...
              {
                node: 'qa',
                message: `AutoGen debugging failed after ${debugResult.attempts} attempts`,
              },
            ],
      };
    }
