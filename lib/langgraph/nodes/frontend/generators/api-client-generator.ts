export function generateApiClient(endpoints: any[], projectId: string, collections: any[] = []): string {
...
${endpoints.map(ep => {
  ...
  const collection = (ep.collection || ep.path.replace(/^\/api\//, '').split('/')[0]).toLowerCase();
  ...
  if (ep.parameters && Array.isArray(ep.parameters) && ep.parameters.length > 0) {
    ...
    const isSearchEndpoint = method === 'GET' && (
...
export function generateEnvFile(projectId: string): string {
