...
async function editorNodeImpl(state: AppGenState): Promise<Partial<AppGenState>> {
  ...
  try {
    ...
    let conversationContext = await getConversationContext(state.projectId);
...
`;
    ...
    // 🧹 CRITICAL: Clean and validate all generated code BEFORE using it
    console.log('[Editor] 🧹 Cleaning and validating generated code...');

    // 🔍 DEBUG: Log BEFORE cleaning to see raw AI output
    console.log('[Editor] 🔍 BEFORE CLEANING:');
    editedFiles.forEach((file, idx) => {
      console.log(`[Editor]   File ${idx + 1}: ${file.path}`);
      console.log(`[Editor]     - Content length: ${file.content.length}`);
    ...
    editedFiles = editedFiles.map(file => {
      ...
      if (errors.length > 0) {
        console.warn(`[Editor] ⚠️ Validation warnings for ${file.path}:`, errors);
      }
    ...
    // 🔍 DEBUG: Log AFTER cleaning to verify
    console.log('[Editor] 🔍 AFTER CLEANING:');
    editedFiles.forEach((file, idx) => {
      console.log(`[Editor]   File ${idx + 1}: ${file.path}`);
      console.log(`[Editor]     - Content length: ${file.content.length}`);
      console.log(`[Editor]     - Content preview: ${file.content.substring(0, 150)}`);
    ...
    // Build detailed feature section if request mentions specific features
...
⚠️ CRITICAL: Maintain consistency with previous decisions from the conversation above!
⚠️ If the user already provided design preferences (colors, fonts, layout), PRESERVE them!
⚠️ If previous edits established patterns (naming conventions, structure), FOLLOW them!
...
🤖 **TASK ANALYSIS:**
Break complex requests into sub-tasks, implement completely, maintain consistency with existing code patterns.
