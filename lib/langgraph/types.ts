...
export interface EditingSession {
  ...
  // Request type (question/edit/feature/design-change/backend-change/frontend-change)
  requestType?:
    | 'question'
    | 'edit'
    | 'feature'
  ...
  // NEW: Subsystem routing fields
  affectsFeatures?: boolean; // Needs PM node
  affectsDesign?: boolean; // Needs UX node
  affectsBackend?: boolean; // Needs Backend node
  affectsFrontend?: boolean; // Needs Frontend node
  startNode?: 'pm' | 'ux' | 'backend' | 'frontend' | 'editor'; // Where to start workflow
  reasoning?: string; // Context Analyzer's routing explanation
...
export interface AppGenState {
  ...
  /** Reasoning context from Input Detector (preserved for downstream nodes) */
  reasoningContext?: {
    intent: string;
  ...
  allRequestedFeatures?: Array<{
    ...
    routes?: Array<{
      // NEW: Routes assigned by PM Node
      path: string;
      purpose: string;
    }>;
    collections?: string[]; // Collections generated by Backend Node for this feature
  }>;
...
export const AppGenAnnotation = Annotation.Root({
  ...
  // UX Designer Output
  designSystem: Annotation<
    'ant-design' | 'tailwind-shadcn' | 'v0-inspired' | 'enhanced-2025' | undefined
  >(),
  ...
  allRequestedFeatures: Annotation<
    | Array<{
        id: string;
        name: string;
        description: string;
  ...
  infrastructureFeatures: Annotation<
    Array<{
      id: string;
      name: string;
      description: string;
