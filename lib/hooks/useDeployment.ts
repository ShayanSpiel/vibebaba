...
export function useDeployment(projectId: string) {
  ...
  const deploy = useCallback(
    async (files: ProjectFile[], backendConfig?: any) => {
      ...
      console.log(
        `[useDeployment] 🚀 Setting status to 'deploying' for project ${projectId} with ${files.length} files...`
      );
      ...
      try {
        ...
        if (!healthCheck || !healthCheck.ok) {
          console.warn(
            '⚠️  Deployment server is not running. Please start it with: cd deployment-server && npm run dev'
          );
