...
export function createBraveSearchTool() {
  return new DynamicStructuredTool({
    name: 'brave_search',
    description:
      'Web search using Brave Search API. Use as FINAL fallback if both Exa and DuckDuckGo fail. Good for general web search.',
    schema: z.object({
    ...
    func: async ({ query, numResults }) => {
      try {
        ...
        return JSON.stringify({
          ...
          source: 'brave',
          results:
            results.web?.results?.slice(0, numResults).map((r: any) => ({
              title: r.title,
              url: r.url,
