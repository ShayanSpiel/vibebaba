...
curl -s -X POST "http://localhost:8090/api/admins" \
  ...
  -d "{\"email\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\",\"passwordConfirm\":\"$ADMIN_PASSWORD\"}" | python3 -m json.tool
...
echo ""
echo "Step 3: Creating projects collection..."
...
echo ""
echo "Step 4: Listing all collections..."
curl -s -H "Authorization: $TOKEN" "http://localhost:8090/api/collections" | python3 -c "import sys, json; data = json.load(sys.stdin); print('Collections:', ', '.join([c['name'] for c in data.get('items', [])]))"
